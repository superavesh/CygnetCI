...
def list_tickets(
    ...
    _perm: dict = Depends(require_permission("tickets", "read")),
    allowed: Optional[List[int]] = Depends(get_allowed_customer_ids),
):
    """List all tickets with optional filters, scoped to the caller's assigned customers"""
    if customer_id:
        require_customer_access(customer_id, allowed)

    q = db.query(models.Ticket)
    if allowed is not None and not customer_id:
        q = q.filter(models.Ticket.customer_id.in_(allowed))
    if status:
...
def create_ticket(
    ...
    """Create a new ticket"""
    require_customer_access(ticket.customer_id, allowed)
...
def update_ticket(
    ...
    data: TicketUpdate,
    db: Session = Depends(get_db),
    _perm: dict = Depends(require_permission("tickets", "update")),
    allowed: Optional[List[int]] = Depends(get_allowed_customer_ids),
):
...
def delete_ticket(
    ticket_id: int,
    db: Session = Depends(get_db),
    _perm: dict = Depends(require_permission("tickets", "delete")),
    allowed: Optional[List[int]] = Depends(get_allowed_customer_ids),
):
    """Delete a ticket"""
    ticket = db.query(models.Ticket).filter(models.Ticket.id == ticket_id).first()
...
def patch_ticket_with_history(
    ticket_id: int,
    data: TicketUpdateWithHistory,
    db: Session = Depends(get_db),
    _perm: dict = Depends(require_permission("tickets", "update")),
    allowed: Optional[List[int]] = Depends(get_allowed_customer_ids),
):
