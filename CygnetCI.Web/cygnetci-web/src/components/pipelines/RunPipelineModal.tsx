...
interface RunPipelineModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRun: (parameters: Record<string, any>, agentId: number | null) => void;
  pipeline: any | null;
...
export const RunPipelineModal: React.FC<RunPipelineModalProps> = ({
  ...
  const handleRun = () => {
    if (validateParams()) {
      ...
      onRun(convertedParams, selectedAgentId);
      onClose();
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            {agents.length === 0 && (
              <p className="text-xs text-yellow-600 mt-1">⚠ No online agents available</p>
            )}
            {!errors['agent'] && agents.length > 0 && (
              <p className="text-xs text-gray-500 mt-1">Choose which agent will execute this pipeline</p>
            )}
          </div>
